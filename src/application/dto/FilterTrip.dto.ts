...
export const FilterTripSchema = z.object({
  ...
  // Sorting
  sortBy: z.enum(['price', 'rating', 'startDate'], {
  ...
  // Trip filters
  arrivalDate: z.string().regex(
    /^\d{4}-\d{2}-\d{2}$/, 
    { message: 'يجب أن يكون تاريخ الوصول بتنسيق YYYY-MM-DD' }
  ).optional(),
  ...
  data => !(data.createdAtMonth && !data.createdAtYear), {
    message: 'يجب توفير السنة عند تحديد شهر الإنشاء',
    path: ['createdAtMonth']
  }
).refine(
  data => !(data.createdAtYear && !data.createdAtMonth), {
  ...
  (data) => !(data.minRating !== undefined && data.maxRating !== undefined) || 
